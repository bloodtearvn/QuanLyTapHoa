...
Public Class frmPrintReceipt
    ...
    Private Sub LoadCustomer()
        Try
            ...
            If dr.Read = True Then
                lblCustomer.Text = "Khách hàng  :  " & dr("CustName")
            Else
    ...
    Private Sub getEmployee()
        Try
            sqL = "SELECT S.StaffID, Fullname FROM STaff as S, POS as P WHERE S.StaffID = P.StaffID AND InvoiceNo = '" & frmPOS.lblInvoiceNo.Text & "'"
            ConnDB()
            cmd = New OleDbCommand(sqL, conn)
            dr = cmd.ExecuteReader(CommandBehavior.CloseConnection)

            If dr.Read = True Then
                lblEmpName.Text = dr("Fullname")
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
        Finally
