...
Public Class frmLoadItem
    ...
    Private Sub Search()
        Try
            sqL = "SELECT ItemNo, itemCode, iDescription, StocksOnHand FROM Item WHERE iDescription LIKE '" & TextBox1.Text & "%' OR itemCode LIKE '" & TextBox1.Text & "%' Order By iDescription"
            ConnDB()
            cmd = New OleDbCommand(sqL, conn)
            dr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()
